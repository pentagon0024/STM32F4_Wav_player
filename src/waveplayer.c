...
__IO uint8_t volume = 90, AudioPlayStart = 0;
...
void WavePlayerStart(void)
{
  char path[] = "0:/";
      ...
      else if(files_flags&0x02)
           {
            WaveRecStatus=2; 
           }
            else if(files_flags&0x04)
                {
                  WaveRecStatus=3; 
                }
                else if(files_flags&0x08)
                    {
                      WaveRecStatus=4; 
                    }
  buffer_switch = 1;
  ...
  else
  {
     if (WaveRecStatus == 1)
    {
      WaveFileName = WAVE_NAME1;
      files_flags&=~1;
    }
    else if (WaveRecStatus == 2)
    {
      WaveFileName = WAVE_NAME2;
      files_flags&=~2;
    }
      else if (WaveRecStatus == 3)
    {
      WaveFileName=WAVE_NAME3;
      files_flags&=~4;
    }
         /*   else if (WaveRecStatus == 4)
    {
      WaveFileName=WAVE_NAME4;
       files_flags&=~8;
    }
            else if (WaveRecStatus == 5)
    {
            ...
            else if (WaveRecStatus == 5)
    {
      ...
      WaveRecStatus=0;
      // увеличиваем WaveRecStatus на единицу                     
        // WaveRecStatus++;
// и играем следующий файл
// Можно проиграть только 5 аудио - файлов и один файл, записанный
// с микрофона
        // if(WaveRecStatus>3) WaveRecStatus=0;
    }    
  }
}
